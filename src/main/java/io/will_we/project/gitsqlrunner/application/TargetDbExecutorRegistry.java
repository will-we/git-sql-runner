...
public class TargetDbExecutorRegistry {
  ...
  public TargetDbExecutor required(String targetId) {
    ...
    List<TargetDbExecutor> list = metaJdbcTemplate.query(
      "SELECT target_id, name, db_type, driver_class_name, driver_jar_path, host, port, database_name, username, password, jdbc_params " +
      ...
      (rs, rowNum) -> {
        ...
        String driverClassName = CompatText.trimToNull(rs.getString("driver_class_name"));
        String driverJarPath = CompatText.trimToNull(rs.getString("driver_jar_path"));
