package io.will_we.project.gitsqlrunner.infrastructure.persistence.jdbc;
...
public class TargetDatabaseRepositoryJdbc implements TargetDatabaseRepository {
  ...
  public int create(TargetDatabase targetDatabase) {
    jdbc.update(
      "INSERT INTO target_database(target_id, name, db_type, driver_class_name, driver_jar_path, host, port, database_name, username, password, jdbc_params, enabled, created_at, updated_at) " +
        "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
      targetDatabase.getTargetId(), targetDatabase.getName(), targetDatabase.getDbType(), targetDatabase.getDriverClassName(), targetDatabase.getDriverJarPath(), targetDatabase.getHost(),
  ...
  public void update(TargetDatabase targetDatabase) {
    jdbc.update(
      "UPDATE target_database SET target_id=?, name=?, db_type=?, driver_class_name=?, driver_jar_path=?, host=?, port=?, database_name=?, username=?, password=?, jdbc_params=?, enabled=?, updated_at=? WHERE id=?",
      targetDatabase.getTargetId(), targetDatabase.getName(), targetDatabase.getDbType(), targetDatabase.getDriverClassName(), targetDatabase.getDriverJarPath(), targetDatabase.getHost(),
      targetDatabase.getPort(), targetDatabase.getDatabaseName(), targetDatabase.getUsername(), targetDatabase.getPassword(),
